...
export interface ChainResult {
  final: string;
  steps: Array<{ op: string; result: string | null; error: string | null }>;
}

export const OPERATIONS: EncoderOperation[] = [
...
async function sha1Hash(msg: string): Promise<string> {
  const buf = new TextEncoder().encode(msg);
...
export async function applyOperation(op: string, input: string): Promise<string> {
  try {
    switch (op) {
      case 'b64-encode':
    ...
    throw new Error(`${op} failed: ${e instanceof Error ? e.message : String(e)}`);
  }
}

export async function applyChain(operations: string[], input: string): Promise<ChainResult> {
  let result = input;
  ...
  for (const op of operations) {
    ...
    try {
      ...
      steps.push({ op, result: null, error: e instanceof Error ? e.message : String(e) });
      break;
    }
  }
