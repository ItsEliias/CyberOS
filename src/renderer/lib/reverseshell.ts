...
export interface ShellEncoding {
  id: string;
  label: string;
}
...
export interface ListenerTemplates {
  nc: string;
  rlwrap: string;
  socat: string;
  ncat: string;
...
export const ENCODINGS: ShellEncoding[] = [
  { id: 'raw',    label: 'Raw' },
  { id: 'base64', label: 'Base64' },
  { id: 'url',    label: 'URL Encoded' },
];

export function generateShell(lang: string, ip: string, port: string): ShellTemplate {
  const templates: Record<string, ShellTemplate> = {
    bash: {
    ...
    powershell: {
      ...
      note: 'PowerShell TCP reverse shell with interactive prompt.'
    },
    perl: {
...
export function generateListener(port: string, protocol = 'nc'): string {
...
export function buildWrappedPayload(lang: string, payload: string, encoding: string): string {
...
export interface StabilisationCommands {
  step1: string;
  step2: string;
  step3: string;
  step4: string;
...
export function getStabilisationCommands(): StabilisationCommands {
  return {
