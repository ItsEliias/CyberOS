...
// ─── Boundary validation ─────────────────────────────────────────────────────
// Every IPC handler accepts arbitrary JSON from the renderer. A malformed
// payload (null, array, wrong type) used to crash the main process with a
// TypeError because handlers assumed shape without checking. These guards
// keep all crashes inside the handler — the IPC contract returns a sane
// error/null instead of taking the whole vault process down.
