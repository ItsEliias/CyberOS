...
const depthShadow = {
  1: 'shadow-elevation-1', 2: 'shadow-elevation-2',
  3: 'shadow-elevation-3', 4: 'shadow-elevation-4',
}

export const Panel = forwardRef<HTMLDivElement, PanelProps>(
  ({ glass, elevated, active, noPad, depth = 1, className = '', children, ...rest }, ref) => {
    const base = 'rounded-md border transition-colors duration-[150ms]'
    const pad  = noPad ? '' : 'p-4'
    let surface: string, border: string, shadow: string
    ...
    return (
      <div ref={ref} className={`${base} ${surface} ${border} ${shadow} ${pad} ${className}`} {...rest}>
        {children}
      </div>
    )
  }
