// GhostVault — Soft lock screen shown when "Require CredVault session" is on
// and the shared SSO state reports CredVault as locked or expired.

import { motion } from 'framer-motion';

interface Props {
  reason?: 'locked' | 'expired';
  onCheck:  () => void;   // re-poll the SSO state right now
}

export default function SSOLockScreen({ reason = 'locked', onCheck }: Props) {
  async function openCredVault() {
    try { await window.ghostvault.openCredVault(); } catch { /* ignore */ }
    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#7bb8ff" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontSize: 16, fontWeight: 600, color: '#e6edf3' }}>
            GhostVault is locked
          </span>
          <span style={{ fontSize: 12, color: '#8b949e', lineHeight: 1.5 }}>
            {reason === 'expired'
              ? 'Your CredVault session has expired.'
              : 'Unlock CredVault to access your notes.'} The lock is enforced because
