...
export default function NoteListItem({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="relative px-4 py-3 cursor-pointer border-b transition-colors"
      style={{
        background   : isSelected ? 'rgba(123,184,255,0.12)' : isActive ? 'rgba(123,184,255,0.08)' : hovered ? 'rgba(255,255,255,0.03)' : 'transparent',
        borderColor  : isDragOver ? '#7bb8ff' : 'var(--border)',
            ...
            <div className="flex gap-1 flex-wrap">
              {tags.map(tag => (
                <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full"
                  style={{ background: 'rgba(123,184,255,0.1)', color: '#7bb8ff' }}>
                  #{tag}
                </span>
              ))}
            ...
            {hovered && !renaming && (
              <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                <button onClick={(e) => { e.stopPropagation(); onPin(); }}
                  ...
                  style={{ color: isPinned ? '#7bb8ff' : 'var(--text-dim)' }}
                  title={isPinned ? 'Unpin' : 'Pin'}>
                  {isPinned ? '★' : '☆'}
                </button>
                <button onClick={(e) => { e.stopPropagation(); setShowExport(v => !v); }}
                  className="text-[10px] px-1.5 py-0.5 rounded transition-colors hover:bg-white/10"
                  style={{ color: 'var(--text-dim)' }} title="Export">
                  ↓
                </button>
