...
export default function SeverityPill({ severity, className = '', showDot = true }: SeverityPillProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-xs text-2xs font-semibold uppercase tracking-wider border ${className}`}
      style={{ background: s.bg, color: s.text, borderColor: s.border }}
    >
      {showDot && (
        <span className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: s.text }} />
      )}
      {s.label}
    </span>
  )
