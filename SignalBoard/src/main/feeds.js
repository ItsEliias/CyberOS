...
export function loadSources() {
    try {
        ...
        const saved = JSON.parse(fs.readFileSync(SOURCES_FILE, 'utf8'));
        const savedIds = new Set(saved.map(s => s.id));
        const merged = saved.map(s => ({ itemCount: 0, errorCount: 0, ...s }));
        DEFAULT_SOURCES.forEach(d => { if (!savedIds.has(d.id))
            merged.push(d); });
        return merged;
    }
    catch {
...
function parseRss(xml, source) {
    try {
        ...
        if (rssItems)
            rawItems.push(...(Array.isArray(rssItems) ? rssItems : [rssItems]));
        if (atomItems)
            rawItems.push(...(Array.isArray(atomItems) ? atomItems : [atomItems]));
        ...
        console.warn(`[SignalBoard] RSS parse error (${source.name}):`, e.message);
        return [];
    }
}
function parseCve(json, source) {
    try {
        ...
        console.warn('[SignalBoard] CVE parse error:', e.message);
        return [];
    }
}
// ─── Main fetch loop ──────────────────────────────────────────────────────────
export async function fetchAllFeeds(allSources, existingItems, ctx, maxPerSource = 30) {
    ...
    await Promise.allSettled(enabled.map(async (source) => {
        ...
        try {
            ...
            if (srcIdx >= 0) {
                ...
                if (volKeys.length > 14)
                    volKeys.slice(0, volKeys.length - 14).forEach(k => delete vol[k]);
            ...
            console.warn(`[SignalBoard] fetch failed (${source.name}):`, e.message);
            if (srcIdx >= 0) {
                const prev = updatedSources[srcIdx];
                updatedSources[srcIdx] = {
                    ...prev,
...
// ─── Alert rule matching ──────────────────────────────────────────────────────
export function applyAlertRules(items, rules) {
    if (!rules.length)
        return items;
...
export function deduplicateItems(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (consumed.has(items[i].id))
            continue;
        const dupes = [];
        for (let j = i + 1; j < items.length; j++) {
            if (consumed.has(items[j].id))
                continue;
...
async function fetchUptimeMonitor(source) {
    const now = new Date().toISOString();
...
// ─── GitHub repo watcher ──────────────────────────────────────────────────────
async function fetchGitHubRepo(source) {
    ...
    try {
        ...
        try {
            ...
            return data2.slice(0, 5).map(c => {
                ...
                const author = commit?.['author']?.['date'];
...
// ─── Vault save ───────────────────────────────────────────────────────────────
export function saveItemToVault(item, vaultPath) {
    try {
        ...
        console.error('[SignalBoard] vault save failed:', e.message);
        return false;
    }
}
