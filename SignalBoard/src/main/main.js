...
import { loadSources, saveSources, loadCache, saveCache, fetchAllFeeds, saveItemToVault, computeTier, applyAlertRules, deduplicateItems, probeFeed } from './feeds';
...
let perSourceTimers = new Map();
let cachedItems = [];
let sources = [];
let settings = DEFAULT_SETTINGS;
let firedNotifIds = new Set();
...
function getRelevanceContext() {
    ...
    const sc = cfg['shared_context'];
    const sb = cfg['signalboard'];
    return {
        lab: sc?.['activeLab'],
        target: sc?.['activeTarget'],
        ip: sc?.['activeIP'],
        customKeywords: sb?.['customKeywords'] ?? [],
        isAuto: true,
    };
}
function writeStatus(items, lastRefresh) {
    try {
        const cfg = readCyberToolsConfig();
...
function saveBookmarks(ids, tags) {
    ensureDir();
...
function maybeFireAlertNotifications(items) {
    const rules = settings.alertRules ?? [];
    if (!rules.length || !settings.notificationsEnabled)
        return;
    let newAlerts = 0;
    items.forEach(item => {
        if (!item.alertMatches?.length)
            return;
    ...
    if (newAlerts > 0)
        updateTrayBadge(alertBadgeCount + newAlerts);
}
function maybeFireNotification(items) {
...
function schedulePerSourceTimers() {
    perSourceTimers.forEach(t => clearInterval(t));
    perSourceTimers.clear();
    sources.forEach(source => {
        if (!source.enabled || !source.pollIntervalMinutes)
            return;
...
ipcMain.handle('feeds:toggle-source', (_e, id) => {
    sources = sources.map(s => s.id === id ? { ...s, enabled: !s.enabled } : s);
    saveSources(sources);
    schedulePerSourceTimers();
    return sources;
});
ipcMain.handle('feeds:update-source', (_e, id, patch) => {
    sources = sources.map(s => s.id === id ? { ...s, ...patch } : s);
    saveSources(sources);
    schedulePerSourceTimers();
    return sources;
});
ipcMain.handle('feeds:add-source', (_e, src) => {
    const id = `custom-${Date.now()}`;
    const newSource = { ...src, id, color: '#ff6b6b', itemCount: 0, errorCount: 0 };
    sources = [...sources, newSource];
    saveSources(sources);
    schedulePerSourceTimers();
    return sources;
});
ipcMain.handle('feeds:delete-source', (_e, id) => {
...
ipcMain.handle('feeds:refresh-source', async (_e, id) => {
...
ipcMain.handle('feeds:test-source', async (_e, url) => {
    try {
...
ipcMain.handle('feeds:probe-feed', async (_e, url) => {
...
ipcMain.handle('feeds:save-bookmarks', (_e, ids, tags) => {
    saveBookmarks(ids, tags);
    return true;
});
ipcMain.handle('feeds:export-bookmarks', async (_e, format, ids, tags) => {
...
ipcMain.handle('feeds:recondesk-target', (_e, itemId) => {
...
ipcMain.handle('config:write-keywords', (_e, keywords) => {
    try {
        const cfg = readCyberToolsConfig();
        const sb = cfg['signalboard'] ?? {};
...
ipcMain.handle('ai:summarise', async (_e, item, apiKey) => {
    if (!apiKey)
        return { ok: false, error: 'No API key configured' };
    try {
        const bullets = await callClaudeApi(item, apiKey);
        cachedItems = cachedItems.map(i => i.id === item.id ? { ...i, aiSummary: bullets } : i);
        saveCache(cachedItems);
...
ipcMain.handle('cve:lookup', async (_e, cveId) => {
    if (cveCache.has(cveId))
        return { ok: true, data: cveCache.get(cveId) };
    try {
...
function computeScoreFromContext(item, ctx) {
    const hay = `${item.title} ${item.summary}`.toLowerCase();
    let score = 0;
    SEC_KEYWORDS.forEach(kw => { if (hay.includes(kw))
        score += 5; });
    if (ctx.lab) {
        const v = ctx.lab.toLowerCase().trim();
        if (v && hay.includes(v))
            score += 10;
    }
    if (ctx.target) {
        const v = ctx.target.toLowerCase().trim();
        if (v && hay.includes(v))
            score += 10;
    }
    if (ctx.ip) {
        if (hay.includes(ctx.ip))
            score += 10;
    }
    ctx.customKeywords?.forEach(kw => { if (kw && hay.includes(kw.toLowerCase()))
        score += 10; });
...
// ─── Claude AI summarise ──────────────────────────────────────────────────────
async function callClaudeApi(item, apiKey) {
