...
const VARIANTS: Record<BadgeVariant, { bg: string; text: string; border: string }> = {
  ...
  accent:  { bg: 'rgba(74,158,255,0.12)', text: 'var(--accent)', border: 'rgba(74,158,255,0.3)' },
}
...
export default function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-xs text-2xs font-medium border ${className}`}
