type BadgeVariant = 'default' | 'success' | 'warning' | 'danger' | 'info' | 'purple' | 'accent' | 'teal'
...
const VARIANTS: Record<BadgeVariant, { bg: string; text: string; border: string }> = {
  ...
  accent:  { bg: 'rgba(45,212,191,0.10)', text: '#2dd4bf', border: 'rgba(45,212,191,0.28)'},
  teal:    { bg: 'rgba(45,212,191,0.10)', text: '#2dd4bf', border: 'rgba(45,212,191,0.28)'},
}
...
export default function Badge({ children, variant = 'default', className = '', dot }: BadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium border ${className}`}
