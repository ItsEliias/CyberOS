...
const VARIANTS: Record<ButtonVariant, string> = {
  ...
  ghost:   'bg-transparent border border-border-default/75 text-text-secondary hover:text-text-primary hover:border-border-strong/60 hover:bg-surface-2',
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'ghost', size = 'sm', loading, disabled, className = '', children, ...rest }, ref) => (
    <button
      ...
      className={`
        inline-flex items-center gap-1.5 font-medium transition-all duration-fast cursor-pointer
