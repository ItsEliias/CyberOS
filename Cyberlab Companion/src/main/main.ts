...
function saveConfig(cfg: Record<string, unknown>): boolean {
  try {
    ...
    // sibling apps reading mid-write (their JSON.parse throws or sees a
    // truncated config — they silently degrade to defaults until the
    // next refresh). tmp+rename keeps readers seeing either the old or
    // the new bytes.
