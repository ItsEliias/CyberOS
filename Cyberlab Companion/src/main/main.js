...
import { saveTokenSecure, loadTokenSecure, clearTokenSecure, fetchHtbStats, fetchThmStats, writeActiveLab, } from './platforms.js';
...
async function checkForUpdates() {
    try {
        ...
        const result = await fetchJSON(cfg.updateCheckUrl || UPDATE_CHECK_URL, {
...
function createWindow() {
    ...
    mainWindow.once('ready-to-show', () => {
        ...
        installPendingActionWatcher('cyberlab', (action) => {
            try {
                mainWindow?.webContents.send('pending-action', action);
            }
            catch { /* ignore */ }
        });
    });
...
function registerIPC() {
    ...
    ipcMain.handle('claude-chat', async (_, payload) => {
        try {
            return { success: true, data: await callClaude(payload) };
        }
        catch (e) {
    ...
    ipcMain.handle('save-session', (_, data) => {
        try {
            ...
            // via "id" like "../../../something". Sessions ids are short
            ...
            // of an active session leaves a truncated/half-parsed JSON, which
            // load-session then quietly drops as null on next launch. The
            // user loses their lab notes for no reason.
            const tmp = fp + '.tmp';
    ...
    ipcMain.handle('scan-vault', (_, vaultPath) => scanVaultForNotes(vaultPath));
    ...
    ipcMain.handle('sync-htb', async (_, htbKey) => {
        if (!htbKey)
            return { success: false, error: 'No HTB API key — add it in Settings first' };
        try {
            ...
            if (authRes.status === 401 || authRes.status === 403)
                throw new Error('Invalid HTB API key');
            ...
            for (const ep of eps) {
                try {
                    ...
                    if (res.status !== 200 || res.raw)
                        continue;
                    const d = res.data;
                    const machines = d?.profile?.owns || d?.owns || d?.machines || d?.data || [];
    ...
    ipcMain.handle('sync-thm', async (_, username) => {
        if (!username)
            return { success: false, error: 'No THM username — add it in Settings first' };
        try {
            const result = await fetchJSON(`https://tryhackme.com/api/user/${encodeURIComponent(username)}`, { headers: { 'Accept': 'application/json' } });
            if (result.raw)
                throw new Error('THM requires auth — use Bulk Import instead');
            if (result.status === 404)
                throw new Error(`THM user "${username}" not found`);
            if (result.status !== 200)
                throw new Error(`THM API returned ${result.status}`);
            const data = result.data;
            const rooms = data.completedRooms || data.rooms || [];
            if (!Array.isArray(rooms) || rooms.length === 0)
                throw new Error('No rooms returned — use Bulk Import');
            const labs = rooms.map((r) => ({
    ...
    ipcMain.handle('save-htb-token', async (_, token) => {
        if (!token || typeof token !== 'string')
            return { success: false, error: 'Empty token' };
        const test = await fetchHtbStats(token);
        if (!test.success)
            return { success: false, error: test.error || 'HTB validation failed' };
        if (!saveTokenSecure(HTB_TOKEN_FILE, token))
    ...
    ipcMain.handle('test-htb-token', async (_, token) => {
    ...
    ipcMain.handle('fetch-htb-stats', async () => {
        ...
        if (!token)
            return { success: false, error: 'Not connected to HTB' };
    ...
    ipcMain.handle('save-thm-token', async (_, payload) => {
        const token = payload?.token || '';
        if (!token)
            return { success: false, error: 'Empty THM token' };
        const test = await fetchThmStats(token, payload.username);
        if (!test.success)
            return { success: false, error: test.error || 'THM validation failed' };
        if (!saveTokenSecure(THM_TOKEN_FILE, token))
            return { success: false, error: 'Failed to encrypt token' };
    ...
    ipcMain.handle('test-thm-token', async (_, payload) => {
        const token = payload?.token || loadTokenSecure(THM_TOKEN_FILE);
        if (!token)
            return { success: false, error: 'No THM token configured' };
    ...
    ipcMain.handle('fetch-thm-stats', async () => {
        ...
        if (!token)
            return { success: false, error: 'Not connected to THM' };
        const cfg = loadConfig();
        return fetchThmStats(token, cfg.thmUsername);
    });
    ipcMain.handle('set-active-lab', (_, payload) => {
    ...
    ipcMain.handle('update-launcher-status', (_, status) => {
        try {
            ...
            if (status.currentLab) {
                ...
                if (fs.existsSync(CONFIG_PATH)) {
                    try {
                        shared = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
                    }
                    catch { }
                }
    ...
    ipcMain.handle('ecosystem-emit', (_, appName, eventType, data) => {
        emitEvent(appName, eventType, data);
        return true;
    });
    ...
    ipcMain.handle('load-lab-tracker', () => {
        try {
            if (fs.existsSync(LAB_TRACKER_FILE))
                return JSON.parse(fs.readFileSync(LAB_TRACKER_FILE, 'utf8'));
        }
        catch { }
        return null;
    });
    ...
    ipcMain.handle('save-knowledge-base', (_, data) => {
        try {
    ...
    ipcMain.handle('save-lab-reviews', (_, data) => {
        try {
            fs.writeFileSync(LAB_REVIEWS_FILE, JSON.stringify(data, null, 2), 'utf8');
            return true;
        }
        catch {
            return false;
    ...
    ipcMain.handle('load-lab-reviews', () => {
        try {
            if (fs.existsSync(LAB_REVIEWS_FILE))
                return JSON.parse(fs.readFileSync(LAB_REVIEWS_FILE, 'utf8'));
        }
        catch { }
        return null;
    });
    ipcMain.handle('export-html', async (_, { content, labName, meta }) => {
        try {
            const { filePath } = await dialog.showSaveDialog({
...
app.on('before-quit', () => {
    ...
    if (autosaveInterval)
        clearInterval(autosaveInterval);
    try {
        saveConfig({ cyberlab_status: { activeSession: null, elapsedTime: null } });
    }
    catch { }
});
