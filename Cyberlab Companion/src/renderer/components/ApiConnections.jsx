...
const HTB_HELP = 'Paste your HackTheBox API v4 token (https://app.hackthebox.com/profile/settings → Create App Token). ' +
    'Stored encrypted via Electron safeStorage. Used to pull rank, owns, and active machines every 5 minutes.';
const THM_HELP = 'TryHackMe has no official public API. Paste either your username (limited public stats) or your full ' +
    'session cookie (e.g. "connect.sid=...") for richer data. Stored encrypted via Electron safeStorage.';
// ── HTB Card ─────────────────────────────────────────────────────────────────
function HtbCard({ onChange }) {
    const [state, setState] = useState({ connected: false, stats: null, error: null });
    ...
    const refresh = useCallback(async () => {
        try {
            ...
            const r = await api.fetchHtbStats();
            setState({
                connected: true,
            ...
            setState({ connected: false, stats: null, error: e.message });
        }
    }, []);
    ...
    async function save() {
        ...
        try {
            const api = window.electronAPI;
            const r = await api.saveHtbToken(token.trim());
            if (r?.success) {
                setStatus('ok');
                setToken('');
    ...
    async function test() {
        ...
        try {
            const api = window.electronAPI;
            const r = await api.testHtbToken(token.trim() || undefined);
            if (r?.success) {
                setStatus('ok');
                if (r.data)
                    setState(s => ({ ...s, stats: r.data, error: null }));
            }
            else {
                setStatus('fail');
    ...
    return (<section className="card space-y-3" style={{ position: 'relative' }}>
      ...
      {state.connected && state.stats && (<div className="grid grid-cols-3 gap-2 text-center text-xs">
      ...
      {state.connected && state.stats?.activeMachines?.[0] && (<div className="text-[11px]" style={{ color: 'var(--text-dim)' }}>
          Active machine: <span className="font-mono" style={{ color: 'var(--accent)' }}>{state.stats.activeMachines[0].name}</span>
        </div>)}

      {!state.connected && (<div className="input-group">
          <label>API Token</label>
          <input type="password" value={token} onChange={e => setToken(e.target.value)} className="w-full font-mono text-xs" placeholder="eyJ0eXAiOiJKV1QiLCJh..."/>
      ...
      <div className="flex gap-2 flex-wrap">
        {!state.connected ? (<button className="btn-accent px-3 py-1.5 text-xs" onClick={save} disabled={busy || !token.trim()}>
            ...
            <button className="btn-ghost px-3 py-1.5 text-xs" onClick={disconnect} disabled={busy} style={{ color: 'var(--error)', borderColor: 'var(--error)' }}>
              Disconnect
            </button>
          </>)}
      </div>

      {state.error && (<div className="text-[11px] font-mono" style={{ color: 'var(--error)' }}>{state.error}</div>)}
    </section>);
}
// ── THM Card ─────────────────────────────────────────────────────────────────
function ThmCard({ onChange }) {
    const [state, setState] = useState({ connected: false, stats: null, error: null });
    const [token, setToken] = useState('');
    ...
    const refresh = useCallback(async () => {
        try {
            ...
            const r = await api.fetchThmStats();
            setState({
                connected: true,
                stats: r?.success ? r.data || null : null,
                error: r?.success ? null : (r?.error || 'Sync failed'),
    ...
    async function save() {
        ...
        try {
            const api = window.electronAPI;
            const r = await api.saveThmToken({ token: token.trim(), username: username.trim() || undefined });
            if (r?.success) {
                setStatus('ok');
                setToken('');
                setState({ connected: true, stats: r.data || null, error: null });
    ...
    async function test() {
        ...
        try {
            const api = window.electronAPI;
            const r = await api.testThmToken({ token: token.trim() || undefined, username: username.trim() || undefined });
            if (r?.success) {
                setStatus('ok');
                if (r.data)
                    setState(s => ({ ...s, stats: r.data, error: null }));
    ...
    return (<section className="card space-y-3">
      ...
      {state.connected && state.stats && (<div className="grid grid-cols-2 gap-2 text-center text-xs">
          <div className="py-1.5 rounded" style={{ background: 'var(--bg3, rgba(13,14,24,0.6))', border: '1px solid var(--border)' }}>
      ...
      {state.connected && state.stats?.partial && (<div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{state.stats.partial}</div>)}

      {!state.connected && (<>
          <div className="input-group">
            <label>Username (optional)</label>
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} className="w-full font-mono text-xs" placeholder="your-thm-username"/>
          </div>
          <div className="input-group">
            <label>Session Cookie or Token</label>
            <input type="password" value={token} onChange={e => setToken(e.target.value)} className="w-full font-mono text-xs" placeholder="connect.sid=s%3A..."/>
            <div className="text-[10px] mt-1" style={{ color: 'var(--text-muted)' }}>
...
function StatusDot({ connected, error }) {
    ...
    return (<span className="inline-flex items-center gap-1.5 text-[10px]" style={{ color: 'var(--text-muted)' }}>
      <span style={{
...
// ── Wrapper ──────────────────────────────────────────────────────────────────
export default function ApiConnections({ onChange }) {
    return (<div id="api-connections" className="space-y-3">
      <div className="flex items-center gap-2">
