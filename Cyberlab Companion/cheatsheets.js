// cheatsheets.js — Bundled offline cheat sheet data
// All data is static JS objects — zero network calls required
...
const CHEATSHEETS = {
    ...
    sections: [
      {
        ...
        items: [
          ...
          { hash: '56 chars hex', type: 'SHA224', example: '', tool: 'hashcat -m 1300' },
          { hash: '64 chars hex', type: 'SHA256', example: '5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8', tool: 'hashcat -m 1400, john --format=sha256' },
          { hash: '96 chars hex', type: 'SHA384', example: '', tool: 'hashcat -m 10800' },
          ...
          { hash: 'aad3b435b51404eeaad3b435b51404ee', type: 'NTLM (empty)', example: 'Empty NTLM hash', tool: 'hashcat -m 1000' },
  ...
  ports: {
    ...
    items: [
      { port: 21, service: 'FTP', description: 'File Transfer Protocol. Check anon login: ftp -n <IP>, anonymous/anonymous. Get file listing: ls -la. Get recursive: wget -r ftp://anon@<IP>/' },
      { port: 22, service: 'SSH', description: 'Secure Shell. Try key auth if creds fail. Brute force: hydra -l user -P wordlist ssh://<IP>. Older versions may be vulnerable.' },
      { port: 23, service: 'Telnet', description: 'Unencrypted remote access. Rare but appears on older/embedded devices. Often no auth or default creds.' },
      { port: 25, service: 'SMTP', description: 'Mail server. Enumerate users: VRFY, EXPN commands. nmap --script smtp-enum-users. May relay mail.' },
      { port: 53, service: 'DNS', description: 'Domain Name System. Zone transfer: dig axfr @<IP> <domain>. Subdomain brute force. Reverse lookup: dig -x <IP>.' },
      ...
      { port: 111, service: 'RPCbind', description: 'NFS/RPC services map. rpcinfo -p <IP> to list RPC programs. Often precedes NFS.' },
      { port: 135, service: 'MSRPC', description: 'Windows RPC endpoint mapper. rpcclient -U "" <IP> for null session. Enum users, groups.' },
      ...
      { port: 389, service: 'LDAP', description: 'Active Directory LDAP. Anonymous queries often allowed. ldapsearch -x -b "DC=..." -H ldap://<IP>.' },
      { port: 443, service: 'HTTPS', description: 'HTTPS web. Check SSL cert for hostnames. Same web checks as port 80. SSL/TLS vulnerabilities.' },
      { port: 445, service: 'SMB', description: 'SMB file sharing. smbclient, enum4linux, crackmapexec. EternalBlue (MS17-010) if unpatched.' },
      { port: 512, service: 'rexec', description: 'Remote execution. rsh/rexec access, often misconfigured.' },
      { port: 513, service: 'rlogin', description: 'Remote login without password if .rhosts misconfigured.' },
      { port: 514, service: 'rsh/syslog', description: 'Remote shell or syslog. .rhosts exploitation.' },
      ...
      { port: 1521, service: 'Oracle DB', description: 'Oracle Database. odat.py for enumeration and exploitation.' },
      ...
      { port: 4444, service: 'Common reverse shell port', description: 'Often used for Metasploit handlers and manual netcat listeners.' },
      { port: 5432, service: 'PostgreSQL', description: 'PostgreSQL database. psql -h <IP> -U postgres. COPY TO/FROM for file read/write.' },
      { port: 5985, service: 'WinRM HTTP', description: 'Windows Remote Management. evil-winrm -i <IP> -u user -p pass. Requires admin rights.' },
      ...
      { port: 9200, service: 'Elasticsearch', description: 'Often unauthenticated. curl http://<IP>:9200/_cat/indices. Data exfiltration.' },
...
function lookupPort(portNum) {
  ...
  return {
    port: portNum,
    service: 'Unknown',
    description: 'No specific information available for this port. Run nmap service version detection: nmap -sV -p ' + portNum + ' <IP>'
  };
}

module.exports = { CHEATSHEETS, lookupPort };
