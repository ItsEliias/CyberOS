// commandbuilder.js — Tool definitions, parameter schemas, command generation
...
// Each tool: id, label, category, description, install, referenceUrl, params[], buildCommand(params)
const TOOLS = [
  ...
  },
  {
    ...
    params: [
      ...
      { id: 'action', label: 'Action', type: 'select', default: '--shares', options: ['--shares','--users','--groups','--pass-pol','--sessions','--loggedon-users','--rid-brute','--sam','--lsa','--ntds','--exec-method smbexec -x "whoami"'] },
    ],
    buildCommand(p) {
  ...
  },
  {
    ...
    buildCommand(p) {
      const ext = { elf:'', exe:'.exe', php:'.php', asp:'.asp', aspx:'.aspx', ps1:'.ps1', sh:'.sh', py:'.py', war:'.war', jar:'.jar', raw:'.bin' };
...
function buildCommand(toolId, params) {
  const tool = getToolById(toolId);
  if (!tool) return `# Unknown tool: ${toolId}`;
  try {
    return tool.buildCommand(params);
  } catch (e) {
    return `# Error building command: ${e.message}`;
  }
}

module.exports = { TOOL_CATEGORIES, TOOLS, getToolById, getToolsByCategory, buildCommand };
